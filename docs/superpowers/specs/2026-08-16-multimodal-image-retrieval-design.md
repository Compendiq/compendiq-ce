...
CREATE TABLE page_image_embeddings (
  ...
  attachment_key TEXT    NOT NULL,   -- filename inside that store, as it is ON DISK: the URL-decoded FINAL SEGMENT of a /api/(local-)attachments/<dir>/<file> src
...
| `domains/llm/services/image-embedding-service.ts` **(new, P2 — shipped)** | llm | `embedPageImages(pageId)` as built: enumerate (`extractImageReferencesFromHtml`) → `resolveAttachmentBytes` → sniff → `readImageDimensions` against `MAX_IMAGE_BYTES`/`MAX_IMAGE_DIMENSION` (**never** `validateImage`, which resizes nothing but also does not answer the width) → sha256-and-model reuse → the external filter, **then** the `rag_images_per_page_max` cap → **one request per image** (`embedImagesVl(cfg, model, [item], …)`; the `messages` path has no batch form, two rows above) → upsert, with a **reconcile** deleting the rows whose `(source, key)` the body no longer references and an **index-identity recheck INSIDE the write transaction**, after the DELETE. Six named skip counters (`missing`/`unsupported`/`tooLarge`/`oversized`/`capped`/`external`) plus `reused`, `removed`, `failed` and, in the worker, `pagesFailed`. Unassigned KEEPS `image_embedding_dirty`; so does a failure. `processDirtyPageImages()` rides the sync cadence beside `processDirtyPages`, under its own `worker:lock:image-embedding-index` with a lifetime holder-epoch timer. |
...
1. After retrieval: `retrievedImages` = up to `rag_answer_max_images` (default
   2) image hits **among the returned pages**, selected **round-robin across
   those pages** (every page's best picture before any page's second, ordered
   within a round by the hit's own similarity — as shipped; a flat best-first
   sort lets one gallery page take every slot, see ADR-025 D8), bytes via
   `attachment-store`, `validateImage`, under a **fixed base64 budget derived
   from `MAX_IMAGE_BYTES`** (~6.7 MB) — a constant rather than a knob (D8b),
   and the backpressure bound for a path that bypasses the queue by design.
...
| P6 ✅ | Diagram/ADR/CLAUDE.md sweep, #1100/#1115 close-out. CLAUDE.md's seven #1115 paragraphs (10,845 words) became two blocks (1,710); ADR-025 gained its **Measured** section; §11 below is closed. | all |
