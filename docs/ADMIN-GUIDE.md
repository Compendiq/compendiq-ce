...
- **Database classification:** `isWiki` is free from the schema — Notion adds a `verification` property to a wiki's row pages and nowhere else — and a wiki always recommends `pages` without sampling anything. Otherwise the tree samples at most `NOTION_ROW_SAMPLE_SIZE` (`5`) rows per database, with `NOTION_ROW_SAMPLE_BUDGET` (`60`) samples across the whole tree build, each probe reading one page of two blocks. A row counts as **empty** when it returns no blocks or only blank paragraphs; a non-paragraph block, a paragraph with text, or a `has_more` page is body content. Any sampled row with content → `rowContent: 'some'` and `recommendedMode: 'pages'`; otherwise any readable empty row → `'none'` and `'table'`; every sample failing → `'unknown'` and `'pages'`. A wiki, a zero-row database, or an exhausted budget lands on `'unknown'` / `'pages'` while spending **zero** requests. The sample is advisory only — the import re-checks every row before it flattens anything.
