...
- **Tree:** `GET /api/notion/tree` builds the hierarchy from Notion Search results and resolves Search-listed block-parent pages without listing every page body. This keeps large workspaces responsive instead of spending up to 80 extra requests before the picker renders. The picker collapses descendants and progressively renders large root collections. A standalone database is **selectable** and carries its own classification: `recommendedMode` (`table` or `pages`), `rowContent` (`none` / `some` / `unknown`), `isWiki`, `rowCount`, and `columns` (the database's property names). `alreadyImported` / `localPageId` are stamped on database nodes exactly as on page nodes, so a re-run recognises a database it already imported.
...
- **Unsupported vs inline:** genuinely unsupported types — `data_source` objects, which the pinned `2022-06-28` API cannot resolve — are non-selectable and labelled **Not supported — stays in Notion**. Search returns a database once; a linked view of that database is not a second tree node. An **inline** database is non-selectable too but is *not* a failure: it reads **Imports inside its page as a table** (`reasonCode` `inline_database` for an inline database object, `child_database` for the block) because its host page's own conversion renders it as a table in that page's body.
- **Run:** `POST /api/notion/import` creates standalone pages (`source = standalone`) under the chosen local space / parent / visibility. `databaseModes` picks each selected database's shape, defaulting to `pages` for a wiki and `table` for everything else. `skip` writes nothing: the database and every selected item beneath it report `skip` with `Database is excluded from import`. `table` writes **one** page — `notion_page_id` is the database id, `body_html` is the rendered columns-by-rows table followed by the lead paragraph, labels are `notion-import` + `database` — and every selected row under it is re-marked `skip` with `Included in the database table`. The picker sends only the database id in table mode; if flatten fails, the importer queries every row and imports them as articles under the container so those bodies are not dropped. `pages` writes a container page for the database and nests the selected row pages under it; a row page's success carries `importedAs: 'article'`, a table `importedAs: 'table'`, an ordinary page `importedAs: 'page'`. Table mode is verified over **every** row rather than the tree's sample: if any row holds page content or cannot be read, the run falls back to `pages` losslessly and reports `Rows have page content — imported as pages instead of one table`. A zero-row database also falls through to the container page but carries **no** downgrade reason — it lost nothing. One page failing does not abort the run. Re-running the same Notion page id reports `already_imported`.
