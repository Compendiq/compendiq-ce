...
- **Run:** `POST /api/notion/import` plans pages and databases together, preserving the selected root's body and allocating parents before children. Actual embedded child pages and databases bring their owned descendants; ordinary links do not. Property-only databases embedded in an imported host become simple parent-page tables, with no extra database or row articles. Row-only later batches recover that decision instead of creating duplicates. Article-bearing inline databases without a body place row articles beneath the host; wiki databases and databases with bodies retain their own article. Every row is checked before flattening; unreadable or nonempty bodies require articles. Imported direct children appear through the existing Child pages macro, once per parent. Explicit **Skip** survives discovery. All planned outcomes, including discovered child failures, are returned; later UI batches merge repeated identities. Per-owner locks serialize overlapping imports, and normalized `notion_page_id` values make re-runs reuse the same articles.
