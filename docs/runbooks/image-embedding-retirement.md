...
|---|---|---|
| `admin_settings.image_analysis_identity` | the retained inference identity (ADR-027 D7) | reverts to its dump-time value, or is deleted if the dump predates the assignment |
| `llm_usecase_assignments` row `usecase = 'image_analysis'` | the vision assignment | same — deleted if the dump predates it |
...
psql --dbname="$POSTGRES_URL" -Atq -o image-analysis-preserve.sql <<'SQL'
SELECT format(
...
# Read it before you rely on it. An EMPTY file means either "this instance
# never assigned a vision model" or "the query matched nothing" — and those
# are not the same fact. Expect one line per image_analysis_* key plus one for
# the assignment.
...
psql --dbname="$POSTGRES_URL" -f image-analysis-preserve.sql
