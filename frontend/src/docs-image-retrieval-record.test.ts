...
const RETRIEVAL_BLOCK = '**Multimodal image retrieval (ADR-027; #1115/ADR-025 retired).**';
const EVAL_BLOCK = '**The image corpus, fixture and `--images` arm axis (#1115 P5; ADR-027).**';
...
describe('09 agrees with itself on how many refusal reasons there are', () => {
  /**
   * Review r2 (minor), re-pointed by #1618 stage 2. P4 added
   * `image_only_context` as a FOURTH reason and P6 left the stand-down
   * sentence three paragraphs below the list reading "all three", so the
   * document contradicted itself and the code in the same section.
   *
   * Stage 2 retired that fourth reason with the image-embedding leg — a set
   * of image-only pages now carries derived `image_analysis` text, so there
   * is context to answer from and nothing to refuse for — and `llm-ask.ts`'s
   * `RefusalReason` union is back to three. The count stays pinned in both
   * directions because the failure mode is unchanged: the list header and the
   * stand-down sentence are edited by different people at different times,
   * and the stand-down sentence is the only place 09 says WHICH reasons a
   * materialised grounding suppresses.
   */
  const flow = read('docs', 'architecture', '09-flow-rag-chat.md');
  const flat = flow.replace(/\s+/g, ' ');

  it('declares three reasons', () => {
    expect(flat).toContain('refuses for **one of three reasons**');
    expect(flat).not.toContain('refuses for **one of four reasons**');
  });

  it('does not then say the otherGrounding stand-down covers four', () => {
    const offenders = flat
      .split(/(?<=[.;]) /)
      .filter((sentence) => /stand[- ]?down covers[^.;]*\ball four\b/i.test(sentence))
      .map((sentence) => sentence.trim().slice(0, 160));
    expect(
      offenders,
      'image_only_context went with the image-embedding leg (#1618 stage 2) — ' +
        'the stand-down covers all three reasons.',
    ).toEqual([]);
  });
});
