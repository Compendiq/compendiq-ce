...
describe.skipIf(!dbAvailable)('resolveUsecase — truth table', () => {
  ...
  it("resolveUsecase refuses every non-inheriting use case", async () => {
    // #1618 retired `image_embedding`, which was the third name on this list;
    // `LlmUsecaseSchema` no longer admits it, so the refusal it used to get
    // here is now a parse failure one layer up.
    await seed();
