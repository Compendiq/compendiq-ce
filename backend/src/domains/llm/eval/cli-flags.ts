...
export const EVAL_USAGE = [
  ...
  '                        arm axis, in the arm, the revision or any held-fixed retrieval knob.',
  ...
  '  --images              measure the IMAGE corpus instead of the text gate — seed the German image',
  '                        corpus through the real intake. Needs --arm B|C: #1115 P5b\'s paired',
  '                        leg-off/leg-on axis measured ADR-025\'s image-embedding leg, and #1618',
  '                        stage 2 retired it. Writes retrieval-eval-arm-<B|C>.json unless --out says',
  '                        otherwise, so it cannot overwrite a text-gate baseline. Implies --lang de',
  '                        and refuses any other; --fts-language still applies to the lexical leg. A',
  '                        --baseline from the other axis is refused. Refuses --deep-search: it',
  '                        reformulates per request, so two arms would be paraphrased separately and',
  '                        the difference would be read as the enrichment\'s.',
  '  --arm B|C             #1614 PR2 / ADR-027 (re-registered B vs C by amendment A-1): run ONE arm of',
  '                        the pre-registered comparison on the image corpus (needs --images).',
  '                        B = candidate with derived chunks (needs the post-#1617 revision,',
  '                        image_analysis assigned, image_analysis_max_output_tokens set and the',
  '                        backfill run on this database — see --backfill-timeout; refused right after',
  '                        the migrations, before any seed, when any of these is absent);',
  '                        C = ablation, authored text only (asserts image_analysis unassigned and no',
  '                        derived rows). Arm A — the legacy image-embedding leg — is RETIRED and',
  '                        refused: it needed a VL embedding endpoint the product no longer has.',
  '                        Writes retrieval-eval-arm-<B|C>.json unless --out says otherwise. A',
  '                        --baseline must be the other arm\'s report of the same corpus, query set,',
  ...
  '',
  'With --images no further variables are read: the image corpus is seeded as text plus attachment',
  'bytes and arm B\'s enrichment comes from the vision-analysis assignment on the database itself.',
].join('\n');
