...
This runbook describes how the Billing team deploys **billing-worker** to production. It
...
3. Announce the deploy in #billing-oncall with the release tag.
4. Verify the staging soak has run for at least thirty minutes.

## Deploying

1. Tag the release: `git tag -a billing-worker-vX.Y.Z -m "release"` and push the tag.
...
- Post the release notes in #billing-oncall.
...
Stop the promotion. Page the on-call engineer via PagerDuty: billing-primary. The detailed
...
- Quarterly: review this runbook with the Billing team and update the rota.
