# Catalog Service Deployment Runbook

> Owner: Catalog team · On-call: #catalog-oncall · PagerDuty: catalog-primary

This runbook describes how the Catalog team deploys **catalog-api** to production. It
...
3. Announce the deploy in #catalog-oncall with the release tag.
4. Verify the staging soak has run for at least thirty minutes.

## Deploying

1. Tag the release: `git tag -a catalog-api-vX.Y.Z -m "release"` and push the tag.
...
- Post the release notes in #catalog-oncall.
...
Stop the promotion. Page the on-call engineer via PagerDuty: catalog-primary. The detailed
...
- Quarterly: review this runbook with the Catalog team and update the rota.
