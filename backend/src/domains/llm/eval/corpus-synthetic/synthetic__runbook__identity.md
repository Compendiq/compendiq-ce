...
This runbook describes how the Identity team deploys **identity-svc** to production. It
...
3. Announce the deploy in #identity-oncall with the release tag.
4. Verify the staging soak has run for at least thirty minutes.

## Deploying

1. Tag the release: `git tag -a identity-svc-vX.Y.Z -m "release"` and push the tag.
...
- Post the release notes in #identity-oncall.
...
Stop the promotion. Page the on-call engineer via PagerDuty: identity-primary. The detailed
...
- Quarterly: review this runbook with the Identity team and update the rota.
