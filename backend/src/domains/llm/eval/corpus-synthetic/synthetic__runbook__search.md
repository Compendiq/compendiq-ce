...
This runbook describes how the Search team deploys **search-indexer** to production. It
...
3. Announce the deploy in #search-oncall with the release tag.
4. Verify the staging soak has run for at least thirty minutes.

## Deploying

1. Tag the release: `git tag -a search-indexer-vX.Y.Z -m "release"` and push the tag.
...
- Post the release notes in #search-oncall.
...
Stop the promotion. Page the on-call engineer via PagerDuty: search-primary. The detailed
...
- Quarterly: review this runbook with the Search team and update the rota.
