...
export async function llmAskRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/llm/ask', { ...LLM_STREAM_RATE_LIMIT, preHandler: requireGlobalPermission('llm:query') }, async (request, reply) => {
    ...
    // key can include the resolved provider+model. This guarantees that admin
    // flips of the chat override do not serve a stale cached response that was
    ...
    const ragCacheKey = buildRagCacheKey(chat.model, question, docIds, {
      includeSubPages,
      pageId: body.pageId,
      externalUrls,
