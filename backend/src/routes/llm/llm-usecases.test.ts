...
describe.skipIf(!dbAvailable)('GET /api/admin/llm-usecases', () => {
  it('returns all 8 rows with resolved blocks (#1417 adds inline completion, #1615 image analysis, #1618 retires image embedding)', async () => {
    ...
    expect(Object.keys(body).sort()).toEqual([
      'auto_tag', 'chat', 'embedding', 'image_analysis', 'inline_completion', 'quality', 'rerank', 'summary',
    ]);
    ...
    // empty sentinel — the stage is disabled, not defaulted (#1104). #1417's
    // inline_completion and #1615's image_analysis follow the same rule.
    for (const u of ['rerank', 'inline_completion', 'image_analysis']) {
