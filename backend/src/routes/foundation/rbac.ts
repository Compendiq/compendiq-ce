...
export async function rbacRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/roles', RBAC_RATE_LIMIT, async () => {
  ...
  fastify.get('/groups', RBAC_RATE_LIMIT, async () => {
    const result = await query<{
      id: number;
      name: string;
  ...
  fastify.post('/groups', RBAC_RATE_LIMIT, async (request, reply) => {
  ...
  fastify.patch('/groups/:id', RBAC_RATE_LIMIT, async (request) => {
  ...
  fastify.delete('/groups/:id', RBAC_RATE_LIMIT, async (request) => {
    const { id } = GroupIdParamSchema.parse(request.params);
  ...
  fastify.get('/groups/:id/members', RBAC_RATE_LIMIT, async (request) => {
    const { id } = GroupIdParamSchema.parse(request.params);
  ...
  fastify.post('/groups/:id/members', RBAC_RATE_LIMIT, async (request, reply) => {
    const { id } = GroupIdParamSchema.parse(request.params);
    ...
    try {
      ...
      if ((err as { code?: string }).code === '23505') {
        throw fastify.httpErrors.conflict('User is already a member of this group');
      }
      throw err;
    }
  ...
  fastify.delete('/groups/:id/members/:userId', RBAC_RATE_LIMIT, async (request) => {
  ...
  fastify.get('/spaces/:key/roles', RBAC_RATE_LIMIT, async (request) => {
  ...
  fastify.post('/spaces/:key/roles', RBAC_RATE_LIMIT, async (request, reply) => {
    const { key } = SpaceKeyParamSchema.parse(request.params);
    ...
    if (roleCheck.rows.length === 0) {
      throw fastify.httpErrors.notFound('Role not found');
    }
  ...
  fastify.delete('/spaces/:key/roles/:assignmentId', RBAC_RATE_LIMIT, async (request) => {
    ...
    if (result.rows.length === 0) {
      throw fastify.httpErrors.notFound('Role assignment not found');
    }
