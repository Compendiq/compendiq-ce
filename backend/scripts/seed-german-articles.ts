...
const spaces: SpaceData[] = [
  ...
  // ==========================================
  // 3. KUBERNETES SPACE (~38 articles)
  // ==========================================
  {
    key: 'K8S',
  ...
  // ==========================================
  // 4. SECRETS MANAGEMENT SPACE (~38 articles)
  // ==========================================
  {
    key: 'SECRETS',
...
function generateFullHtml(article: ArticleData): { html: string; text: string } {
...
async function run() {
  ...
  try {
    ...
    for (const space of spaces) {
      ...
      // Populate full article list (including programmatic topics to hit ~38 per space = 152 total)
      const allArticles: ArticleData[] = [...space.articles];
      const extraTopics = categoryTopics[space.key] || [];

      for (const topic of extraTopics) {
        allArticles.push({
          title: topic,
          icon: space.icon,
          summary: `Tiefgehende technische Dokumentation, Architekturmuster und Best Practices zum Thema ${topic}.`,
          sections: [
            {
              heading: '1. Architektur & Grundlagen',
              content: `Das Konzept von "${topic}" bildet einen integralen Bestandteil moderner Enterprise-Infrastrukturen. In diesem Dokument werden Funktionsweisen, Konfigurationsstandards und Sicherheitsrichtlinien im Detail erläutert.`
            },
            {
              heading: '2. Implementierung & Konfiguration',
              content: `Folgende Konfiguration demonstriert den produktionsreifen Einsatz von ${topic} unter Einhaltung von Sicherheits- und Hochverfügbarkeits-Vorgaben:`,
              code: {
                language: 'bash',
                snippet: `# Status und Validierung von ${topic}\nsystemctl status || kubectl get pods -A || docker ps\n\n# Konfigurationsprüfung und Syntax-Check\necho "Prüfung für ${topic} erfolgreich abgeschlossen"`
              },
            ...
            },
            {
              ...
              content: `Typische Fehlerbilder bei ${topic} umfassen Berechtigungsverweigerungen, Ressourcenengpässe oder Timeout-Probleme. Audit-Logs und Metriken liefern hierzu die primären Anhaltspunkte zur Ursachenanalyse.`
            }
          ]
        });
      }

      console.log(`Writing ${allArticles.length} articles into space ${space.key}...`);

      let sortOrder = 0;
      for (const article of allArticles) {
        sortOrder++;
        const { html, text } = generateFullHtml(article);

        const pageRes = await client.query<{ id: number }>(
          `INSERT INTO pages (
            space_key, title, body_html, body_text, version, source, visibility,
            page_type, created_by_user_id, owner_id, depth, sort_order,
           ...
           RETURNING id`,
          [space.key, article.title, html, text, userId, sortOrder, article.icon]
        );
        ...
        await client.query(
          ...
          [pageId, article.title, html, text]
        );
    ...
    console.log(`\n========================================`);
    console.log(`Successfully generated ${totalArticlesCreated} German technical articles across 4 spaces!`);
    console.log(`========================================\n`);
