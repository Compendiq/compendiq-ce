...
- **Analytics dashboards now export richer PDFs (#303).** The existing ~80-LOC `exportToPdf` helper was replaced with a multi-page renderer: cover page (title / date range / generated-by / instance URL / row count / SHA-256 integrity hash over the canonical-JSON of the data), optional 2×3 KPI grid, paginated data tables with column widths driven by content and per-page footer (`Page N of M`), and an EE-overlay hook (`setPdfRendererOverlay`) that mirrors the `setLlmAuditHook` pattern so enterprise can swap the renderer without touching CE. Each of the four dashboards (`AiUsageDashboard`, `KnowledgeHealthDashboard`, `SearchEffectivenessDashboard`, `ContentGapsDashboard`) now supplies its own KPI list to the cover page. Chart-image rendering (`recharts-to-png`) is intentionally deferred to a follow-up — v0.4 PDFs are tables + KPIs only. (#303)
