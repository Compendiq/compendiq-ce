...
**An empty list names which emptiness it is (#1402 phase 3).** `PagesPage`'s browse-empty block answers three unrelated questions and must not blur them. A filter or a search term emptied it → say which filter, and offer `Clear filters` (the 2026-08-17 harden pass; do **not** add a spaces check there, "no spaces connected" is not why a search returned nothing). Neither is set and the user has no PAT **or** no `selectedSpaces` → `No Confluence spaces connected`, with `Connect Confluence` deep-linking `CONFLUENCE_SETTINGS_PATH`, because the old copy sent someone who had never entered a token to the settings *root* to find the panel themselves. Neither is set and Confluence really is connected → today's copy verbatim, `Go to Settings` / `Create a Page`; that reader has finished with the setup screen. Both halves read the `settings` the component already fetches for its KPIs — no request was added, and `hasConfluencePat` + `selectedSpaces` are the same computed pair the checklist uses one paragraph up. `SpacesTab` follows the same rule: its filter's zero-match state says `No spaces match "…"` instead of the loaded list's `Click "Fetch Spaces"` prompt, which would be a lie.

**A list you cannot search is a list you have to read.** The Keyboard Shortcuts modal is 22 registry rows across four categories plus 11 TipTap ones inside a 60vh scroller, and `SpacesTab` shows every space a PAT can read — dozens to hundreds on a real Data Center instance. Both carry a plain local `nm-input` filter (no request, no URL state: a lookup inside one surface is not a shareable view), matching case-insensitively on `label`, and on `name`/`key` respectively. Two rules hold in both: a **section with no matches disappears whole** — an `<h3>` over empty space claims "no shortcut here", a different thing from "not this one" — and zero matches is an **honest inline line**, never a second `EmptyState` inside a dialog. The shortcuts filter resets on open (Radix unmounts the content, so a stale query would greet the next `?` pre-filtered) and takes focus via `onOpenAutoFocus`, since Radix would otherwise hand it the Close button — the one control this dialog already has a key for.
