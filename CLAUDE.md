...
**No lift, no scale, no in-flow glass.** Hover and press are background/border changes. `translateY` on hover, `scale` on press and `backdrop-blur` on an in-flow pane are all retired. Blur is reserved for modal scrims (`fixed inset-0 bg-black/NN`) and floating `nm-popover-glass` popovers. The latter keep the interactive border, a 95% elevated-surface fill and 10px blur; `prefers-reduced-transparency` makes the fill opaque and removes blur. `workspace-themes.test.ts` measures composited text (≥4.5:1) and edges (≥3:1) over black and white images in both themes: opaque-token contrast alone is insufficient. Dialogs stay opaque `nm-card-elevated`.
