...
**Compendiq** — AI knowledge-base app over Confluence Data Center, multi-LLM (Ollama, OpenAI-compatible). Confluence is **per-user optional**: with `user_settings.confluence_enabled` off the app runs standalone — every feature works against the local corpus and nothing syncs (#1623; see *Confluence Integration Toggle*). Monorepo: `backend/` (Fastify 5 + Postgres + Redis), `frontend/` (React 19 + Vite), `packages/contracts/` (shared Zod schemas).
...
N named `openai-compatible` providers in `llm_providers` table, configured via Settings → AI Models. Each use case (chat / summary / quality / auto_tag / embedding) inherits a default or pins an explicit `provider+model` — **except `rerank` (#1104), which never inherits: unassigned means the rerank stage is disabled** (it targets a Cohere/Jina-style `/v1/rerank` endpoint the default provider cannot serve; `resolveRerankUsecase`, not `resolveUsecase`, and a dedicated `rerank-client.ts` sharing the queue/breaker infra) **and `image_analysis` (#1115, ADR-027 D3/D7), which never inherits for the same reason one rung stronger** — a text-only model answers the request with a fluent description of nothing, so the assignment is probe-gated and unassigned PAUSES the analysis worker (see the multimodal block below) **and `image_analysis` (#1615, ADR-027 D3), the third rung: the assignment IS the egress control** — no page image leaves the host until an admin has saved a provider for it, so unassigned means no new image analysis, never the default or the chat provider; the PUT probes the pair synchronously with the known-content vision probe and refuses with 422 `{reason}` (`no_provider` / `no_model` / `text_only` / `unconfirmed`) before any row is written, `resolveImageAnalysisUsecase`, and the last `true` identity is retained in `admin_settings.image_analysis_identity` across an unassign (pause, not purge). Ollama uses its `/v1` shim — not a separate protocol. Queue + per-provider circuit breakers wrap every outbound call in `openai-compatible-client.ts`.
...
**Multimodal image retrieval (ADR-027; #1115/ADR-025 retired).** ONE design is on record. A generative vision model *analyzes* each referenced raster at ingestion into bounded, versioned text, and `embedPage` embeds that text with the ordinary text embedder as provenance-marked `page_embeddings` rows (`metadata.source = 'image_analysis'`, after every authored chunk, excluded from `page_avg_embedding`, never crossed by sibling assembly), indexed lexically through a per-chunk `chunk_tsv` (migration 117) — one index, one query embed, no third RRF leg, and a text-only chat model can cite the picture. **#1618 stage 2 RETIRED ADR-025's legacy image-embedding leg** (migration 118): `page_image_embeddings`, `pages.image_embedding_dirty`, the `image_embedding` use case and its six `admin_settings` rows, `image-leg-search.ts`, `vl-embedding-client.ts`, the MRL truncation width, the `rag_image_leg_enabled` toggle, the third RRF leg, `degraded_reason = 'image_leg_unavailable'` and the `image_only_context` refusal are all gone. **The basis was "Remove it, nobody was using it in production." — unused in production plus maintenance burden, explicitly NOT a measurement** (ADR-027 A-5): #1619's pre-registered primary (B vs A) was never measured, because arm A needs a VL *embedding* endpoint the owner declined to stand up. Do not describe the retirement as measurement-justified, and do not describe ADR-025's leg as live. ADR-025 is superseded IN FULL by ADR-027, except its **Measured** sections and D8/D8b, which survive as the optional chat attachment. Design of record: ADR-027; operations: `docs/runbooks/image-analysis.md`, and `docs/runbooks/image-embedding-retirement.md` for the cutover and its rollback. **The use case.** `image_analysis` is `rerank`-shaped: it never inherits, and unassigned PAUSES the worker rather than degrading retrieval (D7). Probe BEFORE the assignment row is written — a tri-state vision probe, and a failure is a 422 that refuses the row, because a wrong verdict here fills the corpus with plausible garbage (D3). The retained inference identity is `provider:model@baseUrl` hashed into `page_image_analyses.identity_hash`; a change to it re-pends every row through D5's validity predicate, with payloads KEPT so an unchanged picture comes back `reused` at no vision call (D5/D7). **Intake** (`image-intake.ts`, `image-analysis-worker.ts`). Take the store from the URL PREFIX in `body_html` — `/api/attachments/` vs `/api/local-attachments/` — never from `confluence_id IS NULL` (a relocated page has no `confluence_id` and its bytes in the local store). Skip and COUNT, never resize: SVG, draw.io XML behind a `.png`, over `MAX_IMAGE_BYTES`, over `MAX_IMAGE_DIMENSION`, missing, past `rag_images_per_page_max`, external with the knob off (D10). **`pages.image_analysis_dirty` IS the queue** (D6.2): raise it from every writer that can move an image — the attachment writers through `core/services/image-analysis-dirty.ts`, the `body_html` writers inline in the UPDATE they already own — and clear it only when nothing FAILED, because a skip is a fact about the file and a failure is a fact about the endpoint. Let the reconcile KEEP a `missing` row: clearing a cached file never touches `body_html`, and the lazy re-fetch refills it. The one other thing that removes rows is the #1349 attachment orphan sweep (`attachment-sweep-service.ts`, admin-triggered and dry-run-first), and the two cannot collide — a file it removes is referenced by no body anywhere, and a `missing` row's file IS referenced, so it sits in the sweep's global keep-set. The sweep's rules are stated once, in that module's header; operator view in `docs/ADMIN-GUIDE.md`. Event-driven removal obeys different rules: a hard delete or purge drops `local/<pk>/` and `page-icons/<pk>/` unconditionally, and the shared-keyspace `<pk>/` only when no page claims that `confluence_id` and the directory has aged past a 5-minute grace, consulting NO keep-set (`standalone-attachment-cleanup.ts`); every icon discard rides a COMMITTED `DELETE … RETURNING id`, never a rollback branch. Run the worker under `worker:lock:image-analysis` on the #1612 batch pattern, never the per-user `embedding:lock:*`, and renew its holder epoch from a timer armed for the run's LIFETIME (D13). Sync must NEVER kick it: it has its own repeatable job. **The answer path** (`retrieved-images.ts`; ADR-025 D8/D8a/D8b, ADR-027 D11/D12). **Derived provenance is the only candidate set** — a row's `derived.attachmentKey`, never a whole-set fallback. Read attachment bytes through `core/services/attachment-store.ts`'s `resolveAttachmentBytes`, which applies **no ACL** — legal only after retrieval has applied `visiblePagesPredicate` and the EE per-page filter, which is why the pick is a `domains/llm` service and why `attachment-store.test.ts` walks `src/routes` and fails if any file there names it. Attach bytes only when `getVisionCapability` reads exactly `true` in the caller (`llm-ask.ts`); `false` and `null` both mean text-only. Order the parts text, the USER's own attachment, then the retrieved ones; add ONE system sentence only when a picture was attached. Say NOTHING when a gate shuts: the answer is text-only and UNQUALIFIED, the pictures stay in `sources[]`, and the fact is stated once beside the Retrieval knob (D8). Never count a retrieved image as `otherGrounding` — the pick runs after the refusal decision. Keep `RETRIEVED_IMAGES_BYTE_BUDGET` a CONSTANT derived from `MAX_IMAGE_BYTES` (D8b). Audit COUNTS only, base64 never. Put cited pictures on the `/llm/ask` wire as `kind: 'image'` with `similarity: null` and D12's attachment provenance, appended after the page and web entries, capped at `rag_answer_max_images`; since #1361 the same ENTRY is persisted per assistant turn through the chip allow-list — `toPersistedSources` copies `kind`/`attachmentUrl` together, never singly — and echoed back by `GET /llm/conversations/:id`. **Settings surfaces.** Settings → AI Models → LLM providers carries the **Image analysis (vision)** row — the assignment, the **Last probe** chip and **Re-check**, which re-adopts the identity through D7 with the re-analysis scope disclosed first. Settings → AI Models → Embeddings carries the **Image analysis** progress card. Settings → AI Models → Retrieval carries **Images per page**, **Index external images** and **Images shown to the model**, with a MUTED unassigned note and the controls left enabled (`RetrievalTab.test.tsx`).
...
resolved AND no row written BY THIS PROCESS since boot, which is a restarted pod
or any pod that did not serve the write as much as a first-ever resolve —
reaches the bootstrap rather than the constant 100,
...
("while no `rag_ef_search` row has been read…"): the function reads `process.env` and
...
**The image corpus, fixture and `--images` arm axis (#1115 P5; ADR-027).**
...
`--images` is a CORPUS SELECTOR on the ADR-027 arm axis, not a flag on the
gate and no longer a paired leg switch: **#1618 stage 2 retired the paired
form** with `page_image_embeddings` itself (migration 118), so there is no
second leg to force per query and no in-process A/B. A bare `--images` is
REFUSED before the database is opened — it names `--arm B|C` (ADR-027 "Arms
and revisions"): B is the candidate with derived `image_analysis` chunks live,
C the same revision with `image_analysis` unassigned, and arm A (ADR-025's
legacy leg) is refused like any unknown arm, reproducible only by checking out
its recorded `revisionSha`. One run measures ONE arm into its own report
(`axis: 'images'`, default `--out retrieval-eval-images.json`) and the pairing
happens LATER across files — `assertComparableArms` for the retrieval
endpoints, `judge-arms.ts --unblind` for the answer ones — so everything the
ADR holds fixed is a field of `ArmRunReportSchema` and a refusal there, and
what one arm must carry and another must not (B's vision model and
output-token ceiling) is `armProvenanceProblems`. Seed it through the REAL
intake — bytes on disk under `attachment-store`'s own layout, the body
rewritten by `buildPageImageUrl`, then `embedPage` — because a seeder that
INSERTed rows would measure its own fixture; the analysis backfill runs off
the `image_analysis_dirty` queue the seeder raises. `--deep-search` is refused
here (expansion reformulates per request, so the arms would be paraphrased
separately). Recipe: `docs/runbooks/retrieval-eval.md` ("Arm axis").
**Measured 2026-08-18 (local shim, so plumbing-grade — ADR-025 D11)** is the
RETIRED leg's record and is never re-labelled (ADR-027 A-5): the leg never
cost a page at K ≥ 3 and was a tie at R@1 on a text-easy corpus, `imageHit@1`
was .82, and 2B ≥ 8B at a quarter of the intake cost — full tables in ADR-025
**Measured** §B.
...
Connections belongs below the read-mode article, not on another canvas. Keep
explicit links, hierarchy, and ranked recommendations in separate groups.
`page_relationships` is the evidence, not an invitation to invent an AI reason:
...
**Confluence off is STANDALONE MODE, not a degraded product.** The owner's
ruling, verbatim: *"When confluence is off the app should be in standalone
mode, all features should work just with no sync to confluence."*
`user_settings.confluence_enabled` (migration 119, `BOOLEAN NOT NULL DEFAULT
TRUE`, so every pre-existing row keeps behaving exactly as it did) carries it
per user. Everything keeps working on the local corpus; nothing is pushed to or
pulled from Confluence. Six rules fall out, and none of them is negotiable.

1. **A previously synced page stays fully usable while the integration is
   off** — editable, movable, deletable, AI-usable. The write lands locally and
   is NEVER pushed upstream; the row keeps its `confluence_id` and its history.
2. **`Confluence not configured` is not an answer to a standalone user.** That
   string stays reserved for a Confluence-ENABLED user whose credentials are
   missing or broken. A flag-off user gets local success, not an error.
3. **Credentials are retained across a toggle.** Turning it off clears neither
   `confluence_url` nor `confluence_pat`, so re-enabling needs no re-paste and
   the masked PAT placeholder is still there afterwards.
4. **No surface may prompt for a URL or a PAT while the flag is off** — not the
   PAT banner, not the Getting Started checklist, not the Library empty state,
   not the version-history strip, not the spaces panel. Copy that mentions
   Confluence at all in that state describes the mode; it does not solicit
   setup.
5. **Scheduled and manual sync do not run**, and the spaces/sync surfaces
   **hide** (never merely disable) `Fetch Spaces`, `Sync Selected` and the sync
...
6. **One truth source, and it is read as `=== false`.**
   ...
   read *unknown* as *off* — the one misreading that silently makes a connected
   user's workspace standalone.

**A local write succeeds; only a Confluence-ONLY operation refuses.** With the
flag off a synced page takes the same write path a standalone page takes — the
editor/collab save, `PUT /pages/:id` and the AI-improve apply write the local
row, bump the local version and stamp `local_modified_at`, making **zero**
Confluence calls; `DELETE /pages/:id` removes the local row and leaves the
upstream page alone (trash is standalone-only, so a soft delete there would be
unrestorable). What refuses is the set of operations that have no local
meaning — creating a page *in* a Confluence space, `POST /pages/bulk/sync`,
`POST /embeddings/force-embed-tree` and `POST /pages/:id/relocate` (both
directions: it is defined as create-upstream / delete-upstream) — all with
`Confluence integration is disabled`, never `Confluence not configured`.
`POST /api/sync` answers **200** with that same sentence (no status flicker, no
`SYNC_STARTED` audit row), `GET /api/spaces/available` answers **409**, and a
`selectedSpaces` write arriving while the integration is off answers **422**;
the **400 `Confluence not configured`** is now reachable only by an enabled user
whose credentials are missing or broken. Re-enabling needs no new reconciliation
model: the divergence a standalone stretch produced is handled by the existing
...
import is not attempted at all in standalone mode, and the panel says so ("the
list below is this page's local history") instead of earning the credential
prompt the older status carries.

**It is a MODE — not a credential state, and not `pages.source`.**
`confluenceConnected` still answers only "do stored credentials exist": a user
can be connected and disabled, or disabled and connected, and the response
schema reports both independently. And `source = 'standalone'` classifies one
row's *origin* (the cascade section above); `confluence_enabled` is a per-user
integration *mode* and says nothing about any page's provenance. The collision
of words is unfortunate and the two must not be collapsed.
...
A flat workspace application, not a dashboard. Themes: **Graphite** (dark, `#0F0F10` workspace / `#161617` pane / `#09090A` canvas) and **Paper** (light, `#F8F8F7` workspace / `#FFFFFF` pane / `#EBEAE8` canvas), both neutral, carrying one desaturated **Steel** accent (`#86AEC8` dark / `#3F627C` light) that is the single brand **and** interaction colour. **Paper's panes are pure white and its ramp is NEAR-NEUTRAL (2026-08-30).** The ramp sits at OKLCH hue ~68–70 — the old one was ~250–286, a cool cast that read as screen-grey beside Steel — but at chroma 0.0013–0.0046, i.e. one to three 8-bit steps of red over blue. **Do not describe this as a warm palette: at that chroma the hue is below the perceptual threshold on a normal panel, and a measured critique said so.** The whisper exists to keep the ramp off the cool blue-grey it came from, which is why `workspace-themes.test.ts` still pins red-above-blue on every ramp neutral — the guard defends a direction, not an appearance. Surfaces are an ordered ladder: Canvas `#EBEAE8` (owner-pinned; also the left destination rail and top app header) → Chrome `#F5F5F4` → Workspace `#F8F8F7` → Pane `#FFFFFF`. **States are three tokens, not one:** hover `--color-accent`, press `--color-pressed`, selection `--color-selected`, each a further rung from the pane (Paper 1.081 / 1.140 / 1.193:1; Graphite 1.070 / 1.138 / 1.258:1). One shared token made a hovered tree row identical to the selected one. The floor is **perceptual, not just ordinal** — adjacent steps must clear ΔE-OK 0.014, because Graphite once shipped a press step of 0.0101 that passed every ordering assertion and was invisible in rendered rows. **Embedding is not a hue:** `--color-status-embedding` resolves to `var(--color-foreground)`, so ambient telemetry is body ink plus a progress affordance rather than Steel. **Prose links are underlined at rest** — WCAG G183 needs ≥3:1 against surrounding body ink and Steel measures 1.94:1 there, which no retune can fix. And `/login` carries the one declared exception to the flat-surface rule, via `@utility login-halo`, measured by `login-halo-surface.test.ts` (ADR-010 v0.9, v1.0). The document, the left navigation pane and the context rail are `#FFFFFF`; the frame around them (Canvas — which is also the left destination rail and the top app header — plus Chrome and Workspace), the fills, the borders and the ink carry the whisper. **Canvas is the darkest Paper surface again, and a real grey (v1.1):** it went `#FAFAF9` → `#F4F3F1` when the shell's own frame lines came off and the Canvas/Pane step became the whole card boundary, then to `#EBEAE8` on "more gray" — 1.202:1 on Pane, the same rung `--color-selected` occupies. **That is the floor of the range:** the destination rail's inactive labels are 12px `--color-muted-foreground` on Canvas at 4.51:1 against 1.4.3's 4.5, so a deeper frame must darken the secondary ink first, and a test pins the pair. Graphite still avoids near-black on its reading pane; the "avoid pure white" half of that rule no longer applies to Paper. **Amber is reserved for warning/attention only**, violet marks AI. The craft bar is Linear, Plane and Notion — Compendiq must hold up beside all three.

The two Steel values are tuned independently for their surfaces: Graphite's `#86AEC8` clears 7.67:1 on the document pane and Paper's `#3F627C` clears 6.46:1 on its white pane. `workspace-themes.test.ts` computes those ratios from the tokens rather than pinning hexes, so a retune fails with the measured number instead of a diff; it also pins the ramp's direction (red channel above blue on every neutral) so a cool grey cannot drift back one token at a time, the three state fills' order **and their perceptual separation**, the status hues under simulated protanopia/deuteranopia/tritanopia, that every operable border resolves to a colour rather than `transparent`, that prose links stay underlined, and — because two critiques in a row used this file's comments as claims to verify and found them stale — that no comment in a theme block names a hex the block no longer declares. The eight base roles live in ADR-010 v0.7; Paper's values in v0.8; the state, status and prose-ink corrections in v0.9. Do not scatter equivalent literals across components — that includes chart series, which resolve tokens at render (`src/shared/lib/`), not hexes.

**Depth is a value step and a shape; a hairline only where nothing else states the boundary.** Surfaces are FLAT COLOURS: `--surface-backdrop`, `--surface-card` and `--surface-card-elevated` are plain values, not gradients. Canvas forms the outer frame, Workspace carries navigation, Chrome marks the Library results header band, and content is the Pane one step *up*. That ladder is why the document is the brightest thing on screen and navigation recedes without being dimmed. **Since v0.9 the shell draws no lines at all:** not around the workspace card, not around the context rail, and not under the 48px chrome band across the top of every pane. That band is held by HEIGHT — the sidebar's chrome row, the article context strip and the inspector's tab row all resolve to exactly 48px so the panes start their content on one y, which is why `PageViewPage` and `EditorToolbar` use the full `3rem` and no longer subtract a parent's border. `toolbar-rule-alignment.test.ts` fails both when a row loses `h-12` and when one reinstates a `border-b`; one line back on its own is a rule that starts and stops mid-width. **The content panes inside the shell draw none either (v0.9, second pass):** the two Library results panels and the AI page's options row, message pane and diagram row lost their rings, because a list with a header band, a divider per row and a closing divider states its own extent, and a Pane on Workspace with a radius states its own edge; `app-shell-layout.test.ts`'s `Content panes carry no ring` block fails if one returns. **There are three line weights, and each has one job.** `--color-border` is the app's structural rule — the left navigation's `border-r` (Pane on both sides), a card's own ring (a card paints Pane on a Pane), list-row dividers, the two sticky headers their own content scrolls under (`SettingsLayout`'s title strip — the last `min-h-[calc(3rem-1px)]` in the codebase — and `Editor`'s fallback toolbar), the pane footers a scrolling list ends against, badge and `kbd` edges. It was softened in the second pass to `#222225` / `#EFEEEC`, **~1.15:1 on the Pane**, because after the shell and the content panes went unlined nothing rests on it; `workspace-themes.test.ts` holds a 1.20:1 ceiling and a 1.10:1 floor on it, and the Paper ≥1.35 floor that paid for the near-white frame is retired with its reason. `--doc-rule` is held at the pre-softening value for document content only — `.prose` / `.tiptap` table cells (including the `--tw-prose-*-borders`), the blockquote marker and `hr` — because a data grid is content structure, and at the app rule's weight a `td` (0.6 alpha) measured 1.09:1; a sweep fails if anything outside `.prose` / `.tiptap` borrows it. `--color-border-interactive` is unchanged: anything operable, plus the overlay edge on `nm-card-elevated`, whose Raised surface shares Pane's pure white in Paper. The owner was offered an app-wide quieting of that token and declined it. Do not add a `[data-theme-type="light"]` override for a shell surface; both themes are one token-driven ladder, and retuning the token moves both.

**The authenticated app sits in an inset shell on `md+`.** `--app-chassis` is the viewport ground (`body` overscroll included) and paints the top app header too, completing one continuous outer frame on all four sides. The neutral Chrome token `--app-header-bg` (cool grey in Graphite, near-neutral in Paper) has exactly one home left: the Library results header band, where — since v0.9's second pass took the panel's ring and the row's own `border-b` off — the fill IS that header's boundary. A pane's own first row inherits the pane, which is why both `.app-sidebar .panel-toolbar` and `.app-context-rail .panel-toolbar` are transparent. With no fill and no hairline, a chrome row is read off the control in it — which is why the Pages rail's space selector carries a Workspace fill (`bg-background`, the only surface darker than Pane in BOTH themes at 1.06:1, and the one that keeps hover darkening rather than lightening). The command palette remains available through Ctrl/Cmd+K without a persistent search bar. Left nav and main content sit in a brighter rounded workspace card (`--app-shell-*`) below the header. Outline, Details and Assistant share a detached context rail (`--app-rail-*`) beside that card. **Neither carries a border (v1.1):** the inset, the radius and the Pane-over-Canvas step draw both, `--app-shell-border-width` is deleted, and `app-shell-layout.test.ts` fails if a border in any spelling returns — it holds a 1.08:1 floor under that value step instead, because an unlined card fails silently. Its `--app-rail-gap` gutter is 4px at `md` and 6px at `xl`; the gutter owns the inspector's visible three-dot separator and is the mouse, keyboard and double-click resize target, rather than borrowing an invisible strip from inside the panel. Below `md` inset, radius and gap are 0 (edge-to-edge). Retune those tokens; do not scatter equivalent padding across layout components. **`/login` does not share the frame:** it paints `--app-login-ground` via `@utility login-backdrop` (`#FAFAF9` in Paper, the chassis value in Graphite), because its halo's measured AA floor was calibrated against a near-white ground and the grey frame broke it at 4.013:1 — `login-halo-surface.test.ts` owns that token, so the workspace frame can move without dragging the login hero under AA (ADR-010 v1.1).

**Chassis-only tonal exception (owner request, 2026-09-12; ADR-010 v1.6).** The outer frame carries a static multi-grey wash through `--app-chassis-tones`, using `--app-chassis-highlight`, the unchanged `--app-chassis` base, and `--app-chassis-shade`. `.app-header` is transparent so the image stays continuous across header and rails rather than restarting. Paper ranges from `#F3F3F3` to `#E8E8E8` around `#EDEDED`; Graphite from `#101011` to `#070708` around `#09090A`. The owner's dark-mode follow-up lifts the shared main, left and right pane from `#161617` to `#19191A`, with hover adjusted to `#1D1E1E` so the state stays visible. All three panes use `--color-card`, never local overrides. Paper and login are unchanged. `--surface-backdrop` stays the flat overscroll fallback. Measure rail-label, focus-indicator and unlined-pane contrast against every chassis stop, not just the base.

Pane retunes also require updating the brand tile in `Logo.tsx`, all four
static SVG mirrors and the five raster icon assets. Recheck the dedicated
collaboration-caret palette against both panes; the shell/theme suites alone
do not cover logo parity or caret contrast.

**Exactly one real shadow exists.** `--shadow-overlay` (offset + soft blur) is carried by `nm-card-elevated` and `nm-popover-glass`, for content that genuinely floats above the page: popovers, dropdowns, dialogs, the command palette, toasts. In-page command surfaces such as Library search stay flat and use the shared interactive border for their boundary. Every other in-page pane that wants emphasis earns it from position, spacing and heading weight. The retired `--nm-shadow-*` / `--nm-highlight-*` tokens still exist but resolve to `transparent`, so any missed callsite renders flat rather than leaving one embossed control behind.

**No lift, no scale, no in-flow glass.** Hover and press are background/border changes. `translateY` on hover, `scale` on press and `backdrop-blur` on an in-flow pane are all retired. Blur is reserved for modal scrims (`fixed inset-0 bg-black/NN`) and floating `nm-popover-glass` popovers. Popovers use a softened border with reduced contrast (18% of the interactive edge) and frosted glass with 12px blur in both dark (90% fill) and bright (82% fill) modes so the glass effect is visible in light mode while keeping text readable. `prefers-reduced-transparency` makes the fill opaque and removes blur. Theme tests composite the fill over black and white images in both themes, verifying ≥4.5:1 text contrast. Dialogs stay opaque `nm-card-elevated`.
...
**The `/ai` composer holds the request's controls, and the rail holds New chat (owner request, 2026-09-01).** Each mode now keeps its applicable request controls inside the composer box, in one action row under the field. The full Ask row orders them as attachment trigger, external-URL trigger, **Skill · action**, **Think**, **Deep search**, then Send; Diagram, Generate and Improve omit the controls that do not apply. This follows `DockPanel`'s structural pattern — a horizontally scrolling control row with Send anchored beside it — not its exact control set: the dock has attachment, **Skill · action**, conditional **Deep search**, and Send, and intentionally has no external-URL or **Think** controls. Four things moved to get there and each removed a duplicate or a stray. **`Think` left the page's sticky options row**, which then held nothing, so the strip itself is gone and renders only for a mode with a secondary setting — an empty sticky box still spends its padding and both column gaps out of the message pane's height. It is `ThinkToggle` (`features/ai/`), placed in all four `/ai` composers plus none of the dock's (the dock never had one) because `/llm/ask`, `/llm/improve`, `/llm/generate` and `/llm/generate-diagram` all read the same provider `thinkingMode`; deleting the row without re-placing the chip would have made extended thinking unreachable in three of the four. It keeps the violet accent against Deep search's Steel: same chip geometry, opposite lifetime — `Think` is written to `localStorage`, Deep search must reset on every ask (#1119), and the colour is the only thing telling them apart at a glance. **Deep search's on-screen caveat became the dock's popover**: the three facts stay one click away and stay the control's `aria-describedby`, rather than printing a permanent two-line paragraph above the field. **The two attachment triggers became one** (`ComposerAttachmentPicker`, see `docs/architecture/04-frontend-structure.md`). **And the page-header New chat is gone**: `AiConversationsSidebar` carries it in both of its states, so no width loses the action, and `AiAssistantPage.test.tsx` fails on any button named *New chat* returning to the heading row. This supersedes #1361 amendment item 2 (the 2026-08-22 decision that put it there).
...
The inspector's Details tab groups provenance, document health, read-only labels, notes, and page actions; below `md` it lives in the page-inspector sheet. The Confluence link stays with provenance and Notes precedes secondary actions. The editor's `TagPopover` chip remains the in-flow label editor while writing, not a duplicate Details editor. Health follows indexing and analysis states independently: an absent error message cannot turn `failed` into success, and human verification does not certify AI readiness. Details mounts lazily and stays hidden rather than unmounting across tab switches, retaining local note/reply drafts; its page key discards them on navigation. The inspector opts into `QualityScoreBadge`'s keyboard/touch disclosure; list badges remain passive because their rows are already buttons. Its portalled details absorb Escape and close when Details becomes inactive.

Notes grows with its content up to the inspector's 420px cap; never restore a minimum-height floor for empty or short lists. Hide empty-list guidance while composing, but retain loading/error feedback and populated threads. Keep the thread region keyboard-focusable with an inset `nm-focus-ring`, metadata on the 13px `text-xs` scale, and actions at 32px; wrap header and thread actions on narrow sheets rather than shrinking their labels.
...
Batch sizes live in `admin_settings.quality_batch_size` / `summary_batch_size`,
edited under Settings → AI Models → Workers; the former batch-size env vars
are ignored. The controls preserve edits made during a save, distinguish
unreadable settings from cached settings, and retain Retry focus through
recovery. Use the shared `nm-input` treatment, not structural border tokens.
...
Department text fallback must not rewrite embedding, rerank or image analysis.
