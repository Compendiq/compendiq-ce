...
**The authenticated app sits in an inset shell on `md+`.** `--app-chassis` is the viewport ground (`body` overscroll included). The header (logo, Find, alerts, user) paints that grey chassis — it is chrome, not document. Left nav and main content sit in a brighter rounded workspace card (`--app-shell-*`) below it. Outline, Details and Assistant share a detached context rail (`--app-rail-*`, gutter `--app-rail-gap`) beside that card. Below `md` inset, radius and gap are 0 (edge-to-edge). Retune those tokens; do not scatter equivalent padding across layout components.
