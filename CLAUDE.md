...
A flat workspace application, not a dashboard. Themes: **Graphite** (dark, `#0F0F10` workspace / `#161617` pane / `#09090A` canvas) and **Paper** (light, `#FAF7F3` workspace / `#FFFFFF` pane / `#F3EEE9` canvas), both neutral, carrying one desaturated **Steel** accent (`#86AEC8` dark / `#3F627C` light) that is the single brand **and** interaction colour. **Paper is a WARM neutral and its panes are pure white (2026-08-30).** Its ramp sits at OKLCH hue ~68–70 — the old ramp was hue ~250–286, a cool cast that read as screen-grey beside Steel — and the document, the left navigation pane and the context rail are `#FFFFFF`, with the warmth carried by the frame around them (Canvas, Chrome, Workspace) plus fills, borders and ink. Graphite still avoids near-black on its reading pane; the "avoid pure white" half of that rule no longer applies to Paper. **Amber is reserved for warning/attention only**, violet marks AI. The craft bar is Linear, Plane and Notion — Compendiq must hold up beside all three.
