...
A flat workspace application, not a dashboard. Themes: **Graphite** (dark, `#0F0F10` workspace / `#161617` pane / `#09090A` canvas) and **Paper** (light, `#F8F8F7` workspace / `#FFFFFF` pane / `#F4F3F1` canvas), both neutral, carrying one desaturated **Steel** accent (`#86AEC8` dark / `#3F627C` light) that is the single brand **and** interaction colour. **Paper is a WARM neutral and its panes are pure white (2026-08-30).** Its ramp sits at OKLCH hue ~68–70 — the old ramp was hue ~250–286, a cool cast that read as screen-grey beside Steel — at one to three 8-bit steps of red over blue: the hue is felt, not seen. The whole ramp is fitted to two owner values — a pure white Pane and the Canvas grey — and the four surfaces are an ordered ladder, Canvas `#F4F3F1` → Chrome `#F5F5F4` → Workspace `#F8F8F7` → Pane `#FFFFFF`. **State fills carry a floor:** hover/selected `#F2F2F1` at 1.12:1 against Pane and pressed `#EBEBEA` at 1.19:1, because an earlier round pinned both to `#FDFDFD` (1.02:1) and rows whose hover is only a fill had no hover at all. `workspace-themes.test.ts` holds that floor, the surface order, and Canvas's exact owner-pinned value (ADR-010 v0.8/v0.9). The document, the left navigation pane and the context rail are `#FFFFFF`; the frame around them (Canvas — which is also the left destination rail and the top app header — plus Chrome and Workspace), the fills, the borders and the ink carry the warmth. **Canvas is the darkest Paper surface again (2026-08-31, v0.9):** it spent one day above Workspace at `#FAFAF9`, and when the workspace card and context rail lost their hairlines the Canvas/Pane step became the whole card boundary — `#FAFAF9` measured 1.04:1 against white, `#F4F3F1` measures 1.11:1, matching the 1.10:1 Graphite gets from its own step. Graphite still avoids near-black on its reading pane; the "avoid pure white" half of that rule no longer applies to Paper. **Amber is reserved for warning/attention only**, violet marks AI. The craft bar is Linear, Plane and Notion — Compendiq must hold up beside all three.
