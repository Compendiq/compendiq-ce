...
**Depth is a value step and a shape, and a hairline only where nothing else states the boundary.** Surfaces are FLAT COLOURS: `--surface-backdrop`, `--surface-card` and `--surface-card-elevated` are plain values, not gradients. Canvas forms the outer frame, Workspace carries navigation, Chrome marks internal panel toolbars, and content is the Pane one step *up*. That ladder is why the document is the brightest thing on screen and navigation recedes without being dimmed. Since v0.9 the shell's own frames are value plus radius with **no border at all** — see the inset-shell paragraph below — and the lines that remain are the ones carrying information no fill does: the 48px chrome rule across the top of every pane, the left navigation's `border-r` (Pane on both sides), a card's border (a card paints Pane on a Pane), and `--color-border-interactive` on anything operable. Do not add a `[data-theme-type="light"]` override for a shell surface; both themes are one token-driven ladder, and retuning the token moves both.
