...
A flat workspace application, not a dashboard. Themes: **Graphite** (dark, `#0F0F10` workspace / `#161617` pane / `#09090A` canvas) and **Paper** (light, `#F9F7F6` workspace / `#FFFFFF` pane / `#FDFDFD` canvas), both neutral, carrying one desaturated **Steel** accent (`#86AEC8` dark / `#3F627C` light) that is the single brand **and** interaction colour. **Paper is a WARM neutral and its panes are pure white (2026-08-30).** Its ramp sits at OKLCH hue ~68–70 — the old ramp was hue ~250–286, a cool cast that read as screen-grey beside Steel — at chroma 0.002–0.007: the hue is felt, not seen. **Two tokens are owner-pinned pure neutrals outside that ramp — `--app-chassis` and `--color-accent`, both `#FDFDFD`.** The second is the hover *and* selected fill, and at 1.02:1 on a white pane it no longer carries either state: selection lives on `nav-selection`'s interactive outline and ink, an interactive card's hover on its border, and a bare `hover:bg-accent` row has near-invisible hover by construction (ADR-010 v0.8). The document, the left navigation pane and the context rail are `#FFFFFF`; the frame around them (Canvas — which is also the left destination rail and the top app header — plus Chrome and Workspace), the fills, the borders and the ink carry the warmth. **Canvas is near-white and no longer the darkest step:** Paper's order is Chrome → Workspace → Canvas → Pane, so the workspace card is read off its hairline rather than off a value step against the gutter. Graphite still avoids near-black on its reading pane; the "avoid pure white" half of that rule no longer applies to Paper. **Amber is reserved for warning/attention only**, violet marks AI. The craft bar is Linear, Plane and Notion — Compendiq must hold up beside all three.
