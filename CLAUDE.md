...
**The article has a reading measure, and it is `rem`.** The document column is 1200px and every block used to fill it — measured in Chromium, ~99 characters per line at 1280px, ~101 at 1512 and ~133 at 1920, roughly twice the comfortable maximum on the surface the product exists to read. `--measure-article` (40rem, ~76 characters at the article's 16px) is carried by the block **children**, not the container: that needs no negative margins and lets a table, code block or diagram keep the full column, which is why `max-w-none` stays on both prose containers. `.article-measure` is on the reader **and** the editor so what you read and what you write cannot drift apart, and `.article-document > *` measures the title, labels and AI summary too — constraining only the prose left the title ~100px to the left of its own paragraphs and the summary card *wider* than the article it summarises. A measured `<input>` also needs `block`: auto margins are a no-op on an inline box, so the title took the right width and stayed left-aligned. `NewPagePage` renders its own title beside the same shared `Editor`, so it carries the same class.

**It is an allow-list of prose (`p, h1–h6, ul, ol, dl, blockquote, hr`), never a deny-list of wide blocks.** The first version measured `> *` and exempted wide selectors, and three of those matched nothing the app renders: TipTap wraps every React node view in `div.react-renderer.node-<name>`, so `.drawio-nodeview` and `[data-type='mermaidBlock']` sit a level deeper. Worse, the reader and editor disagree on shape — a code block is a bare `<pre>` in the reader but a node-view wrapper in the editor (`TitledCodeBlock`) — so one exemption list clamped code while editing and not while reading. An unrecognised block keeping the full column is safe; a missed exemption silently shrinks a diagram.

**Never express the measure in `ch`.** `ch` resolves against the element's own font-size, so one shared `64ch` gives an h2 at 20px a wider box than a paragraph at 16px — headings, subheadings and body each land on a different left edge and the centred column visibly comes apart. `article-reading-measure.test.ts` fails on a `ch` value, on a reintroduced `> *`, on a non-tag selector in the allow-list, and on any callsite dropping its class; `article-measure-dom.test.tsx` **renders both components** and asserts the real `.tiptap` child shapes, which is the only thing that catches a selector matching nothing — jsdom performs no layout but it does report the DOM.
...
**Edit-mode toolbar.** `EditorToolbar` lives in its own module; `Editor.tsx` re-exports it and keeps the three context strips. It carries **twelve controls**, not the thirty-one it once had: a block-type control, the five marks, three list toggles, two colour pickers and one Insert menu, with undo/redo at the far end. The long tail is behind the two menus *with names beside it* — the flat row shipped two different actions under the same `ListTree` glyph and no label, which is the failure mode a wall of icons has. Nothing was removed; `EditorToolbar.test.tsx` asserts the Insert menu item by item against what the flat row carried, so a restructure cannot quietly drop one.
