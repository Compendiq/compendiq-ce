...
A flat workspace application, not a dashboard. Themes: **Graphite** (dark, `#0F0F10` workspace / `#161617` pane / `#09090A` canvas) and **Paper** (light, `#F8F8F7` workspace / `#FFFFFF` pane / `#EBEAE8` canvas), both neutral, carrying one desaturated **Steel** accent (`#86AEC8` dark / `#3F627C` light) that is the single brand **and** interaction colour. **Paper is a WARM neutral and its panes are pure white (2026-08-30).** Its ramp sits at OKLCH hue ~68–70 — the old ramp was hue ~250–286, a cool cast that read as screen-grey beside Steel — at one to three 8-bit steps of red over blue: the hue is felt, not seen. The whole ramp is fitted to two owner values — a pure white Pane and the Canvas grey — and the four surfaces are an ordered ladder, Canvas `#EBEAE8` → Chrome `#F5F5F4` → Workspace `#F8F8F7` → Pane `#FFFFFF`. **State fills carry a floor:** hover/selected `#F2F2F1` at 1.12:1 against Pane and pressed `#EBEBEA` at 1.19:1, because an earlier round pinned both to `#FDFDFD` (1.02:1) and rows whose hover is only a fill had no hover at all. `workspace-themes.test.ts` holds that floor, the surface order, and Canvas's exact owner-pinned value (ADR-010 v0.8/v0.9). The document, the left navigation pane and the context rail are `#FFFFFF`; the frame around them (Canvas — which is also the left destination rail and the top app header — plus Chrome and Workspace), the fills, the borders and the ink carry the warmth. **Canvas is the darkest Paper surface again (v0.9)** and a real grey at 1.20:1 on Pane: it went `#FAFAF9` → `#F4F3F1` when the shell's frame lines came off and the value step became the whole card boundary, then to `#EBEAE8` on "more gray". **That is the floor of the range** — the destination rail's 12px labels are `--color-muted-foreground` on Canvas at 4.51:1 against 1.4.3's 4.5, so a deeper frame must darken the secondary ink first, and a test pins the pair. Graphite still avoids near-black on its reading pane; the "avoid pure white" half of that rule no longer applies to Paper. **Amber is reserved for warning/attention only**, violet marks AI. The craft bar is Linear, Plane and Notion — Compendiq must hold up beside all three.

The two Steel values are tuned independently for their surfaces: Graphite's `#86AEC8` clears 7.67:1 on the document pane and Paper's `#3F627C` clears 6.46:1 on its white pane. `workspace-themes.test.ts` computes those ratios from the tokens rather than pinning hexes, so a retune fails with the measured number instead of a diff; it also pins Paper's warm ramp (red channel above blue on every neutral), so a cool grey cannot drift back in one token at a time. The eight base roles and their exact values live in ADR-010 v0.7; do not scatter equivalent literals across components.
