...
**Multimodal image retrieval (#1115, ADR-025).** Two designs are on record: ACTIVE versus CANDIDATE. **Active (ADR-025, the current release, every rule in this paragraph):** text keeps the Phase-1 text embedder; a vision-language model embeds *images* into a separate `page_image_embeddings` index, and the question is embedded once per space. The two are not merged: VL text retrieval is a measured regression against the text model, and one space would force every text embed through vLLM's chat-embeddings wire shape (ADR-025 D1; the local parity gate agreed — both VL checkpoints clear `bge-m3`, neither beats Qwen3-Embedding-4B, ADR-025 **Measured** §A). **Candidate (ADR-027, epic #1611, packages #1615–#1619; #1615–#1617 merged, #1618/#1619 open):** a generative vision model *analyzes* each referenced raster at ingestion into bounded, versioned text, and `embedPage` embeds that text with the ordinary text embedder as provenance-marked `page_embeddings` rows (`metadata.source = 'image_analysis'`, after every authored chunk, excluded from `page_avg_embedding`, never crossed by sibling assembly), indexed lexically through a per-chunk `chunk_tsv` — one index, one query embed, no third RRF leg, and a text-only chat model can cite the picture. ADR-027 supersedes ADR-025 D1 on scope and reverses D6; everything else below stays live until #1618 retires it after #1619's pre-registered A/B/C gate (owner-confirmed 2026-09-15, ADR-027 "Owner decisions"). Do not describe the legacy leg as retired, and do not claim the candidate improves RAG — that is the hypothesis #1619 measures. Active design of record: `docs/superpowers/specs/2026-08-16-multimodal-image-retrieval-design.md` (superseded in part); operations: `docs/runbooks/image-index.md`. **The active use case.** `image_embedding` is `rerank`-shaped: it never inherits, and unassigned means the leg is off (D3). Reach the model only through `vl-embedding-client.ts` — a `messages` array with a trailing empty `assistant` turn and `continue_final_message: true` — never `generateEmbedding`'s `{model, input}`, which skips the chat template and pools a different position (D4, `vl-embedding-client.test.ts`). Probe BEFORE the assignment row is written: `probeImageEmbedding` blocks and a failure is a 422 that refuses the row, because a wrong verdict here fills an index with plausible garbage (`llm-usecases-image-embedding.test.ts`); the candidate's `image_analysis` copies exactly this pre-write shape with the tri-state vision probe (ADR-027 D3). The index identity is `provider:model@baseUrl#dims` over the RESOLVED model, and a change to it — or to the probed width — TRUNCATEs, retypes, reindexes and re-dirties every page (D7/D12, `image-embedding-index.integration.test.ts`). Send the MRL truncation width on every image-side call (`admin_settings.image_embedding_target_dimensions`, one reader in `core/services/image-embedding-target-dimensions.ts`) and let the probe refuse a server that ignores it (D5). A server upgraded in place behind the same URL is invisible to every signal in the code: that one is a manual re-scan (runbook §8). **Intake** (`image-embedding-service.ts`, runbook §5). Take the store from the URL PREFIX in `body_html` — `/api/attachments/` vs `/api/local-attachments/` — never from `confluence_id IS NULL` (a relocated page has no `confluence_id` and its bytes in the local store). Skip and COUNT, never resize: SVG, draw.io XML behind a `.png`, over `MAX_IMAGE_BYTES`, over `MAX_IMAGE_DIMENSION`, missing, past `rag_images_per_page_max`, external with the knob off (D10). Reuse a row whose sha256 and model still match, at no request at all. Keep `image_embedding_dirty` RAISED while the use case is unassigned: the flag is the queue. Clear it only when nothing FAILED — a skip is a fact about the file, a failure is a fact about the endpoint. Raise it from every writer that can move an image: the attachment writers through `core/services/image-embedding-dirty.ts`, the `body_html` writers inline in the UPDATE they already own (`image-embedding-dirty.integration.test.ts`); the candidate's `pages.image_analysis_dirty` is raised from the SAME writer list by #1616, coexisting until #1618. Let the reconcile KEEP a `missing` row — clearing a cached file never touches `body_html`, and the lazy re-fetch refills it; the one other thing that removes index rows is the #1349 attachment orphan sweep (`attachment-sweep-service.ts`, admin-triggered and dry-run-first), and the two cannot collide: a file it removes is referenced by no body anywhere, and a `missing` row's file IS referenced, so it sits in the sweep's global keep-set. The sweep's rules are stated once, in that module's header; operator view in `docs/ADMIN-GUIDE.md`. Event-driven removal obeys different rules: a hard delete or purge drops `local/<pk>/` and `page-icons/<pk>/` unconditionally, and the shared-keyspace `<pk>/` only when no page claims that `confluence_id` and the directory has aged past a 5-minute grace, consulting NO keep-set (`standalone-attachment-cleanup.ts`); every icon discard rides a COMMITTED `DELETE … RETURNING id`, never a rollback branch. Run the worker under `worker:lock:image-embedding-index`, never the per-user `embedding:lock:*`, and renew its holder epoch from a timer armed for the run's LIFETIME (`image-embedding-worker.integration.test.ts`); the candidate's worker is `worker:lock:image-analysis` on the #1612 batch pattern (ADR-027 D13). **The active leg** (`image-leg-search.ts`, fused in `rag-service.ts`; runbook §6, `docs/architecture/09-flow-rag-chat.md`). Gate on four conditions and do NO retrieval work when shut — `imageLeg` not `false`, `rag_image_leg_enabled`, the use case assigned, the table non-empty. Fuse it as a page-denominated THIRD RRF leg: a page's best image ranks it once, so image count cannot beat image quality (#1106's rule). Spend ONE VL call per request, bounded at 3s, the kNN under its own 2s `statement_timeout`. Run it on deep search's ORIGINAL question only. Give an image-only page its `chunk_index 0` row, or its title flagged `imageTextSynthesized`. Keep an image-ONLY row out of `computeRetrievalConfidence` entirely and never let a cross-modal similarity feed the number: `weak_match` is unchanged, `no_context` is not — a page the leg reaches stands it down. Record a bypass as `degraded_reason = 'image_leg_unavailable'` and let a text-side reason win the one column. Share `visiblePagesPredicate` with the vector leg rather than copying it. Budget a SECOND vector-pool connection per hybrid search; raise `PG_VECTOR_POOL_MAX` before enabling the leg on a busy instance. Put matched pictures on the `/llm/ask` wire as `kind: 'image'` with `similarity: null`, appended after the page and web entries, capped at four; the candidate keeps that entry shape and adds attachment provenance to it (ADR-027 D12). Since #1361 the same ENTRY is persisted per assistant turn through the chip allow-list — `toPersistedSources` copies `kind`/`attachmentUrl` together, never singly — and echoed back by `GET /llm/conversations/:id`. **The answer path** (`retrieved-images.ts`; D8/D8a/D8b, runbook §7). Read attachment bytes through `core/services/attachment-store.ts`'s `resolveAttachmentBytes`, which applies **no ACL** — legal only after retrieval has applied `visiblePagesPredicate` and the EE per-page filter, which is why the pick is a `domains/llm` service and why `attachment-store.test.ts` walks `src/routes` and fails if any file there names it. Attach bytes only when `getVisionCapability` reads exactly `true` in the caller (`llm-ask.ts`); `false` and `null` both mean text-only. Order the parts text, the USER's own attachment, then the retrieved ones; add ONE system sentence only when a picture was attached. Say NOTHING when a gate shuts: the answer is text-only and UNQUALIFIED, the pictures stay in `sources[]`, and the fact is stated once beside the Retrieval knob (D8). Never count a retrieved image as `otherGrounding` — the pick runs after the refusal decision. Refuse `image_only_context` when every returned row is `imageTextSynthesized` AND zero parts were attached AND nothing else grounds the turn; `every`, never `any` (D8a, `llm-ask.test.ts`). Keep `RETRIEVED_IMAGES_BYTE_BUDGET` a CONSTANT derived from `MAX_IMAGE_BYTES` (D8b). Audit COUNTS only, base64 never. The answer cache key gains a 15th component (`retrieved-images.test.ts`). This optional attachment survives the candidate cutover, re-sourced from derived-chunk provenance and falling back to the legacy hits until #1618 (ADR-027 D11). **Settings surfaces (active).** Settings → AI Models → LLM providers carries the **Image embedding** row — the assignment, **Truncate to N dimensions (MRL)**, the **Last probe** chip and **Re-check**, which on a width or endpoint change rebuilds rather than merely reporting. Settings → AI Models → Embeddings carries the **Image index** card (`ImageIndexCard.test.tsx`). Settings → AI Models → Retrieval carries the **Image retrieval** group — **Image leg**, **Images per page**, **Index external images**, **Images shown to the model** — with a MUTED unassigned note and the controls left enabled (`RetrievalTab.test.tsx`). The candidate replaces the first two with **Image analysis (vision)** and an **Image analysis** progress card and retires the Image leg toggle; the last two knobs and **Images shown to the model** survive.
